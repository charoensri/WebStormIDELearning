...
const REFRESH_TICK = "2026-04-27-22-30";
...
app.get('/patients', async (c) => {
  ...
  const totalResult = await db.select({ count: patients.patientId }).from(patients);
  const totalCount = totalResult.length; // Simplified count for SQLite/Drizzle MVP
  const totalPages = Math.ceil(totalCount / limit);

  return c.html(<PatientList patients={allPatients} currentPage={page} totalPages={totalPages} />)
})
...
app.get('/ailments', async (c) => {
  ...
  return c.html(<AilmentList ailments={allAilments} />)
})

// Therapy Catalog Page
app.get('/therapies', async (c) => {
  const allTreatments = await db.select().from(treatments);
  return c.html(<TreatmentList treatments={allTreatments} />)
})
...
app.get('/visits', async (c) => {
  ...
  const state = c.req.query('state'); // RESOLVED or UNRESOLVED
  ...
  return c.html(<VisitList visits={filteredVisits.reverse()} title={title} />)
})
